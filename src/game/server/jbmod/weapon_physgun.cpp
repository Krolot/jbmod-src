...
ConVar phys_gunmass( "phys_gunmass", "200", FCVAR_REPLICATED | FCVAR_NOTIFY );
ConVar phys_gunvel( "phys_gunvel", "400", FCVAR_REPLICATED | FCVAR_NOTIFY );
ConVar phys_gunforce( "phys_gunforce", "20e20", FCVAR_REPLICATED | FCVAR_NOTIFY );
...
public:
	...
	bool MakeConstraint( CBaseEntity *pObject, CGravityPellet *pHeld, CGravityPellet *pBond )
	{
...
void CGravControllerPoint::AttachEntity( CBasePlayer *pPlayer, CBaseEntity *pEntity, IPhysicsObject *pPhys, const Vector &position )
{
...
private:
	...
	freezelist_t m_activeFreeze[MAX_FREEZE];
	int			m_pelletCount;
	int			m_freezeCount;
...
BEGIN_SIMPLE_DATADESC( freezelist_t )
...
void CWeaponGravityGun::SortPelletList( void )
{
	m_pelletCount = 0;
	for ( int j = 0, i = 0; j < MAX_PELLETS; j++ )
	{
		pelletlist_t tmp = m_activePellets[j];
		if ( !tmp.pellet )
		{
			i++;
			continue;
...
void CWeaponGravityGun::SortFreezeList( void )
{
	m_freezeCount = 0;
	for ( int j = 0, i = 0; j < MAX_FREEZE; j++ )
	{
		freezelist_t tmp = m_activeFreeze[j];
...
void CWeaponGravityGun::FreezeObject()
{
...
void CWeaponGravityGun::UnfreezeObjects()
{
	SortFreezeList();
